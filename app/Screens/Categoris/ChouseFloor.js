...
class ChouseFloor extends Component {
    ...
    async componentDidMount(){
      ...
      console.log('get __GoTo from chooseBlock', this.props.lottypes);
    }

    getBlok = () =>{
      const item = this.props.item
      const items = this.props.prevItems
      const lot_type = this.props.lottypes
      {isMount ?
          fetch(urlApi+'c_product_info/getBlok/'+items.db_profile+'/'+items.entity_cd+'/'+items.project_no+'/'+items.tower,{
              method: "POST",
              headers : this.state.hd,
              body: JSON.stringify({lot_type})
          }).then((response) => response.json())
          .then((res)=>{
              if(!res.Error){
                  const resData = res.Data
                  this.setState({blok : resData})
                  _storeData("@FloorData", resData);
              } else {
    ...
    clickUnitEnquiry() {
        Actions.UnitEnquiryProjectPage({
          items : this.props.item,
          prevItems : this.props.prevItems,
          goTotheItems: this.props.gotoTheItem
        });
        this.setState({ click : true})
    }
    render() {
